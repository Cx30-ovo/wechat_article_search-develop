...
from wechat_rpa.runtime import control_panel_runtime as panel
...
class ControlPanelTests(unittest.TestCase):
    ...
    def test_manual_window_options_are_recorded_in_run_snapshot(self) -> None:
        state = panel.ControlState()
        state.history = panel.RunHistory(self.temp_path / "run_history.json")
        with patch.object(panel, "collect_preflight", return_value=blocked_preflight()), patch.object(
            panel, "PANEL_LOG_PATH", self.temp_path / "control-panel.log"
        ...
        record = state.history.recent(1)[0]
        self.assertEqual(record["parameters"]["max_accounts"], 3)
        self.assertEqual(record["parameters"]["start_account"], "厦门晚报")

    def test_scheduled_job_uses_saved_start_account_and_max_accounts(self) -> None:
        state = panel.ControlState()
        state.history = panel.RunHistory(self.temp_path / "run_history.json")
        state.config.update(
            {
        ...
        record = state.history.recent(1)[0]
        self.assertEqual(record["parameters"]["max_accounts"], 8)
        self.assertEqual(record["parameters"]["start_account"], "海西晨报")
    ...
    def test_collector_command_adds_known_url_stop_when_enabled(self) -> None:
        """定时批量任务默认应带上“遇到已采集文章就停止当前公众号”。"""
        output_dir = self.temp_path / "known-url-stop"
        enabled = panel.build_collector_command(
            output_dir,
            {
                "max_articles": 5,
                "scan_range": "today",
        ...
        disabled = panel.build_collector_command(
            output_dir,
            {
                "max_articles": 5,
                "scan_range": "today",
                "metrics": "share",
                "stop_after_known_url": False,
            },
        )

        self.assertIn("--stop-after-known-url", enabled)
        self.assertNotIn("--stop-after-known-url", disabled)

    def test_collector_command_carries_account_window_options(self) -> None:
        """手动任务应把起始公众号和账号数量传给采集器。"""
        output_dir = self.temp_path / "manual-window"
        command = panel.build_collector_command(
            output_dir,
            {
                "max_articles": 7,
        ...
        self.assertEqual(command[command.index("--start-account") + 1], "厦门晚报")
        self.assertEqual(command[command.index("--max-accounts") + 1], "3")

    def test_collector_command_omits_account_window_when_unset(self) -> None:
        """定时任务或全量手动任务不应追加账号窗口参数。"""
        output_dir = self.temp_path / "manual-full"
        command = panel.build_collector_command(
            output_dir,
            {
                "max_articles": 7,
        ...
        self.assertNotIn("--start-account", command)
        self.assertNotIn("--max-accounts", command)

    def test_account_window_options_are_validated(self) -> None:
        self.assertEqual(panel.validate_max_accounts(0), 0)
        self.assertEqual(panel.validate_max_accounts("5"), 5)
        self.assertEqual(panel.validate_max_accounts(None), 0)
        self.assertEqual(panel.validate_start_account(" 厦门日报 "), "厦门日报")
        self.assertEqual(panel.validate_start_account(None), "")
        with self.assertRaises(ValueError):
            panel.validate_max_accounts("abc")
        with self.assertRaises(ValueError):
            panel.validate_max_accounts(201)

    def test_portable_start_script_does_not_disable_vl_fallback(self) -> None:
        """便携启动脚本必须允许 .env 中配置的 Qwen-VL 在本地 OCR 失败后兜底。"""
        script = (
            panel.RPA_DIR / "scripts" / "windows" / "start-rpa.ps1"
    ...
    def test_save_config_persists_schedule_ranges(self) -> None:
        ...
        self.assertEqual(config["max_accounts"], 5)
        self.assertEqual(config["start_account"], "厦门晚报")
