const elements = {
  ...
  startAccount: document.querySelector("#startAccount"),
  startAccountOptions: document.querySelector("#startAccountOptions"),
  maxAccounts: document.querySelector("#maxAccounts"),
  allAccounts: document.querySelector("#allAccounts"),
  accountScopeNote: document.querySelector("#accountScopeNote"),
...
function renderCurrentConfig() {
  ...
  const startText = start ? `“${start}”` : "第一个";
  const allSelected = elements.allAccounts.checked;
  const count = allSelected ? 0 : Number(elements.maxAccounts.value || 0);
  let note;
  if (!total) {
    note = "正在读取公众号列表…";
  } else if (!startMatched) {
    note = `公众号列表共 ${total} 个；未找到起始公众号“${start}”，请从列表中选择。`;
  } else if (allSelected) {
    note = `公众号列表共 ${total} 个；将从${startText}开始，采集全部 ${total} 个。`;
  } else {
    note = `公众号列表共 ${total} 个；将从${startText}开始，最多采集 ${count || total} 个。`;
  }
  elements.accountScopeNote.textContent = note;
  elements.accountScopeNote.classList.toggle(
    "warning",
    Boolean(total && start && !startMatched)
  );
...
async function loadAccountChoices() {
  try {
    const payload = await request("/api/accounts?limit=1000&offset=0");
    accountNames = [...new Set((payload.items || []).map((item) => item.name).filter(Boolean))];
    renderCurrentConfig();
  } catch (_) {
    // 账号列表加载失败时仍允许从第一个开始手动采集。
    renderCurrentConfig();
  }
}
...
function renderAccountOptions(filterText) {
  ...
  if (keyword && !keyword.includes("第一个") && !matches.length) {
    ...
    empty.textContent = "未找到匹配公众号";
    fragment.appendChild(empty);
  }
  elements.startAccountOptions.replaceChildren(fragment);
}

function accountComboboxOpen() {
  renderAccountOptions(elements.startAccount.value);
  elements.startAccountOptions.hidden = false;
  elements.startAccount.setAttribute("aria-expanded", "true");
}

function accountComboboxClose() {
  elements.startAccountOptions.hidden = true;
  elements.startAccount.setAttribute("aria-expanded", "false");
}

function selectAccountOption(value) {
  elements.startAccount.value = value;
  accountComboboxClose();
  renderCurrentConfig();
}

function accountComboboxClickOption(event) {
  const option = event.target.closest(".account-option");
  if (!option) return;
  selectAccountOption(option.dataset.value || "");
}
...
function renderSavedSchedule(config) {
  ...
  savedScheduleOptions = {
    ...
    max_accounts: Number(config.max_accounts || 0),
    start_account: config.start_account || "",
    scheduled_time: scheduledTime,
  };
  elements.savedSchedulePreview.textContent = `已保存计划：${scheduleRangeSummary(savedScheduleRanges)}；指标=${metricsLabels[config.metrics]}，每账号上限=${config.max_articles} 篇，起始=${config.start_account || "第一个"}，账号数=${config.max_accounts || "全部"}。`;
}
...
function openRunConfirm(source) {
  ...
  [
    ...
    `起始公众号：${options.start_account || "第一个"}`,
    `采集账号数量：${options.max_accounts || "全部"}`,
  ].forEach((text) => {
...
elements.maxAccounts.addEventListener("change", () => {
  elements.allAccounts.checked = Number(elements.maxAccounts.value || 0) === 0;
  elements.maxAccounts.disabled = elements.allAccounts.checked;
  renderCurrentConfig();
});
elements.allAccounts.addEventListener("change", () => {
  elements.maxAccounts.disabled = elements.allAccounts.checked;
  if (elements.allAccounts.checked) elements.maxAccounts.value = 0;
  renderCurrentConfig();
});
elements.startAccount.addEventListener("input", () => {
  if (!elements.startAccountOptions.hidden) renderAccountOptions(elements.startAccount.value);
  renderCurrentConfig();
});
elements.startAccount.addEventListener("focus", accountComboboxOpen);
elements.startAccount.addEventListener("click", accountComboboxOpen);
elements.startAccount.addEventListener("keydown", (event) => {
  if (event.key === "Escape") {
    accountComboboxClose();
  } else if (event.key === "Enter") {
    const first = elements.startAccountOptions.querySelector(".account-option");
    if (first) selectAccountOption(first.dataset.value || "");
  }
});
elements.startAccountOptions.addEventListener("mousedown", (event) => event.preventDefault());
elements.startAccountOptions.addEventListener("click", accountComboboxClickOption);
document.addEventListener("mousedown", (event) => {
  if (!event.target.closest(".account-combobox")) accountComboboxClose();
});
