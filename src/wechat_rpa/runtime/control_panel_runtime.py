...
from wechat_rpa.paths import PROJECT_ROOT, STATIC_DIR
...
def build_collector_command(output_dir: Path, options: dict[str, Any]) -> list[str]:
    ...
    if str(options.get("start_account") or "").strip():
        command.extend(["--start-account", str(options["start_account"]).strip()])
    if int(options.get("max_accounts") or 0) > 0:
        command.extend(["--max-accounts", str(int(options["max_accounts"]))])
    if options.get("stop_after_known_url"):
        command.append("--stop-after-known-url")
    return command
...
def recover_sogou_preflight() -> dict[str, Any]:
    ...
    try:
        ...
        from wechat_rpa.runtime.collector_runtime import open_sogou_from_wechat_main
...
class ControlState:
    ...
    def _load_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "max_accounts": validate_max_accounts(saved.get("max_accounts", 0)),
            "start_account": validate_start_account(saved.get("start_account", "")),
            ...
            "stop_after_known_url": bool(
                saved.get("stop_after_known_url", DEFAULT_CONFIG["stop_after_known_url"])
            ),
        }
    ...
    def save_config(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        config = {
            ...
            "max_accounts": validate_max_accounts(payload.get("max_accounts", 0)),
            "start_account": validate_start_account(payload.get("start_account", "")),
            "scan_range": fallback_range,
            "schedule_ranges": validate_schedule_ranges(
            ...
            "stop_after_known_url": bool(
                payload.get("stop_after_known_url", DEFAULT_CONFIG["stop_after_known_url"])
            ),
        }
    ...
    def start_job(
        ...
        with self.lock:
            ...
            selected_max_accounts = validate_max_accounts(
                self.config["max_accounts"] if max_accounts is None else max_accounts
            )
            selected_start_account = validate_start_account(
                self.config["start_account"] if start_account is None else start_account
            )
            options = {
                "source": source,
                "max_articles": count,
                ...
                "max_accounts": selected_max_accounts,
                "start_account": selected_start_account,
                "stop_after_known_url": bool(
                    self.config.get(
                        "stop_after_known_url",
                        DEFAULT_CONFIG["stop_after_known_url"],
                    )
                ),
            }
...
def validate_max_accounts(value: Any) -> int:
    """校验一次手动任务最多采集的公众号数量；0 表示全部。"""
    if value is None or str(value).strip() == "":
        return 0
    try:
        number = int(value)
    except (TypeError, ValueError) as exc:
        raise ValueError("采集账号数量必须是整数") from exc
    if not 0 <= number <= 200:
        raise ValueError("采集账号数量必须在 1 到 200 之间，0 表示全部")
    return number


def validate_start_account(value: Any) -> str:
    """起始公众号名称允许为空，空表示从列表第一个开始。"""
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            if self.path == "/api/run":
                ...
                requested_max_accounts = payload.get("max_accounts")
                requested_start_account = payload.get("start_account")
                ...
                STATE.add_log(
                    ...
                    f"起始公众号={requested_start_account or '第一个'}，"
                    f"账号数量={requested_max_accounts or '全部'}。",
                )
