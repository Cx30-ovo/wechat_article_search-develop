...
& $uvExe pip install --python $venvPython --index-url $indexUrl --only-binary :all: -r (Join-Path $projectRoot "requirements.txt")
if ($LASTEXITCODE -ne 0) {
    throw "Failed to install dependencies. Review the network, proxy, and package index settings."
}
& $uvExe pip install --python $venvPython --no-deps --editable $projectRoot
if ($LASTEXITCODE -ne 0) {
    throw "Failed to install the local wechat_rpa package."
}
